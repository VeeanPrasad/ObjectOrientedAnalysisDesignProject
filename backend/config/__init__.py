...
_current = getattr(sys.modules['backend.config.settings'], '{0}Config'.format(APP_ENV))()
...
def as_dict():
   res = {}
   for atr in [f for f in dir(backend.config) if not '__' in f]:
       val = getattr(backend.config, atr)
       res[atr] = val
   return res